...
def pipeline(spark: SparkSession) -> None:
    ...
    df_DeltaSource_1 = DeltaSource_1(spark)
    ...
    df_Limit_2 = Limit_2(spark, df_DeltaSource_1)
...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/StreamingTestPipeline_1")
    
    MetricsCollector.start(
        spark = spark,
        pipelineId = spark.conf.get("prophecy.project.id") + "/" + "pipelines/StreamingTestPipeline_1"
    )
    pipeline(spark)
