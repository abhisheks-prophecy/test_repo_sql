...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/PYTHON_COMPLEX")
    ...
    MetricsCollector.start(
        spark = spark,
        pipelineId = spark.conf.get("prophecy.project.id") + "/" + "pipelines/PYTHON_COMPLEX"
    )
    pipeline(spark)
