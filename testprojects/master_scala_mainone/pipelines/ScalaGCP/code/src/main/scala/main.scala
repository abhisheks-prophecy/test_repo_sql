...
object Main {
  ...
  def graph(context: Context): Unit = {
    val df_Source_5 = Source_5(context).interim("graph",
                                                "QQ_f4SrARPaP0ZtL6uV0r",
                                                "FtKm0kyfkwIxab-8yOffM"
    )
    Lookup_2(context, df_Source_5)
    val df_Source_9 = Source_9(context).interim("graph",
                                                "xoDRuSduB1niuIF8PP3ct",
                                                "AJqWz1Up0SmtM70ipV65i"
    )
    val df_FlattenSchema_1 = FlattenSchema_1(context, df_Source_9).interim(
      "graph",
    ...
    val df_Source_3 = Source_3(context)
      ...
      .cache()
    val df_Source_4 = Source_4(context).interim("graph",
                                                "9MW3M6D_dPk34335bQHHC",
                                                "iZ223SCz_w9vGNJJkCIw-"
    )
    val df_Source_6 = Source_6(context).interim("graph",
                                                "A7YBjCffwys4LPAleAKpC",
                                                "bsEyEzvvLYNF6CFSyeX9o"
    )
    val df_Source_7 = Source_7(context).interim("graph",
                                                "nCk_vxm7W80JgQ2ANSHA_",
                                                "DqVz4ONoErO0RNOWar0yr"
    )
    val df_Source_8 = Source_8(context).interim("graph",
    ...
    val df_Source_2 = Source_2(context)
      ...
      .cache()
    val df_ComplexExpression = ComplexExpression(context, df_Source_2).interim(
      "graph",
      "2tRCXGkA-6TfjEnFofIJq",
      "MwSfNu4URv3g0PC7kc9CR"
    )
    val df_Script_2 = Script_2(context, df_Source_4).interim(
      "graph",
    ...
    val df_Source_1 = Source_1(context)
      ...
      .cache()
    val df_Filter_1 = Filter_1(context, df_Source_1).interim(
      "graph",
      "gNCO_k3OESC15dRefnTjD",
      "7ecj16KrYrMc5jCMOyV_-"
    )
    val df_Limit_1 = Limit_1(context, df_Source_1).interim(
      "graph",
      "BtjgWEFk-IrCWsqN3RqDF",
      "vz8yOBdktTG02eFYUsCr_"
    )
    val df_OrderBy_1 = OrderBy_1(context, df_Source_1).interim(
      "graph",
    ...
    val df_Source_14 = Source_14(context).interim(
      "graph",
    ...
    withSubgraphName("graph", context.spark) {
      withTargetId("Target_2", context.spark) {
        Target_2(context, df_Source_14)
      }
    }
    val df_Repartition_1 = Repartition_1(context, df_Source_1).interim(
      "graph",
    ...
    val df_Source_11 = Source_11(context).interim("graph",
                                                  "Fmg6g-ViOm77hFxIpAPch",
                                                  "ee0X8XILMHhyMro4U1_V7"
    )
    val df_Source_15 = Source_15(context).interim(
      "graph",
      "vmRKV0Nd6-lh2PBG9DCyM$$r1IXlDV-WCE_ANbMUvQcs",
      "RiRMBmFnYhsz_jKLJYl4P$$UkVrz2ppXuwUgrdRSOXCa"
    )
    val df_Reformat_2 = Reformat_2(context, df_Source_15).interim(
      "graph",
    ...
    val df_SetOperation_1 =
      SetOperation_1(context, df_Source_11, df_Source_11).interim(
        "graph",
        "6MGeoO_3CAkDyZvPYAGOY$$W7ZInyQSxiNrP-XuCd9hK",
        "9nSC-MM8SEHYGTWCQwmW8$$82qHa1CcCeW1MGcYp5jb4"
      )
    val df_Join_1 = Join_1(context, df_Source_1, df_Source_1).interim(
      "graph",
    ...
    val df_Reformat_3 = Reformat_3(context, df_Source_1).interim(
      "graph",
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/ScalaGCP")
    spark.sparkContext.hadoopConfiguration
      .set("hadoop_config1", "hadoop_config1_value")
    spark.sparkContext.hadoopConfiguration
      .set("hadoop_config2",      "hadoop_config2_value")
    MetricsCollector.start(spark, "pipelines/ScalaGCP")
    graph(context)
