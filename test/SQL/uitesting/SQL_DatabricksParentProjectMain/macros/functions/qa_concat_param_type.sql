...
or (
  array_join (array ('hello', null, 'world'), ' ', ',') like '%hello%'
)
...
or map_contains_key (
  map_filter (map (1, 0, 2, 2, 3, -1), (k, v) -> k > v),
  3
)
or map_contains_key (
...
or (
  named_struct ('a', 1, 'b', 2) in (
    named_struct ('a', 1, 'b', 1),
    named_struct ('a', 1, 'b', 3)
  )
)
or (not true)
or regexp(
  '%SystemDrive%\\Users\\John',
  '%SystemDrive%\\\\Users.*'
)
or array_contains (
...
or array_contains (transform(array(1, 2, 3), x -> x + 1), 1)
or map_contains_key (
  transform_keys (
...
or h3_boundaryasgeojson(599686042433355775) is not null
or hex(h3_boundaryaswkb(599686042433355775)) is not null
or h3_centerasgeojson('8009fffffffffff') is not null
or ARRAY_SIZE(h3_compact(h3_kring(599686042433355775, 2))) is not null
or h3_distance('85283447fffffff', '8528340ffffffff') is not null
or h3_hexring('85283473fffffff', 1) is not null
{% endmacro %}
